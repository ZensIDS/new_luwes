...
<section class="content"><div class="box box-primary">
    ...
    <div class="box-body table-responsive"><table class="table table-bordered table-striped table-condensed"><thead><tr><th>Outlet</th><th>Produk</th><th>Diskon Reguler</th><th>Diskon Tambahan</th><th>Margin</th><th>Aktif</th><th>Aksi</th></tr></thead><tbody>
    @forelse($prices as $price)<tr><td>{{ $price->outlet?->name }}</td><td>{{ $price->product?->code }} — {{ $price->product?->name }}</td><td>{{ $price->disc_brand_value }}{{ $price->disc_brand_type === 'percentage' ? '%' : '' }}</td><td>{{ $price->disc_tambahan_value ?? $price->disc_toko_value }}{{ ($price->disc_tambahan_type ?? $price->disc_toko_type) === 'percentage' ? '%' : '' }}</td><td>{{ $price->margin_value }}{{ $price->margin_type === 'percentage' ? '%' : '' }}</td><td>{{ $price->is_active ? 'Ya' : 'Tidak' }}</td><td><a class="btn btn-xs btn-warning" href="{{ route('outlet-prices.edit', $price) }}">Edit</a><form action="{{ route('outlet-prices.destroy', $price) }}" method="POST" style="display:inline">@csrf @method('DELETE')<button class="btn btn-xs btn-danger" onclick="return confirm('Hapus harga ini?')">Hapus</button></form></td></tr>@empty<tr><td colspan="7" class="text-center">Belum ada master harga.</td></tr>@endforelse
